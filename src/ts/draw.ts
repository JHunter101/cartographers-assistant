...
function refillMarket(
  type: 'skill',
  deck: string[], // Modify the type of deck
  market: string[],
  size: number,
): GameProgress {
...
function smoothShape(shape: number[][]): number[][] {
  const size = shape.length;
  const newShape: number[][] = emptyGird(3);
...
function thresholdShape(shape: number[][], thresholdCount: number): number[][] {
  const size = shape.length;
  const newShape: number[][] = emptyGird(3);
  const flatList: number[] = shape.flat();
...
function emptyGird(size: number): number[][] {
  return Array.from({ length: size }, () => Array(size).fill(0));
}

function createShape(size: number): number[][] {
  let shape: number[][] = emptyGird(3);
...
function createExploreCard(): {
  ...
  exploreData['types'] = randUniqueItems(
    ['forest', 'village', 'water', 'farm'],
    choiceTypeCount,
  );

  const maxTypeShapeLength = Math.max(
    exploreData['types'].length,
    exploreData['shapes'].length,
  );
  ...
  isRuined = isRuined.concat(
    Array(maxTypeShapeLength - isRuined.length).fill(
      isRuined[isRuined.length - 1],
    ),
  );

  isCoined = isCoined.concat(
    Array(maxTypeShapeLength - isCoined.length).fill(
      isCoined[isCoined.length - 1],
    ),
  );
  ...
  exploreData['types'] = exploreData['types'].concat(
    Array(maxTypeShapeLength - exploreData['types'].length).fill(
      exploreData['types'][exploreData['types'].length - 1],
    ),
  );
  exploreData['shapes'] = exploreData['shapes'].concat(
    Array(maxTypeShapeLength - exploreData['shapes'].length).fill(
      exploreData['shapes'][exploreData['shapes'].length - 1],
    ),
  );
...
function drawCard(): void {
  ...
  if (gridContainer) {
    ...
    for (let i = 0; i < exploreData['types'].length; i++) {
      ...
      const extraType: ('ruined' | 'coins')[] = ['ruined', 'coins'];
  ...
  const seasonDisplayAddition = document.getElementById(
    'season-display-addition',
  ) as HTMLInputElement;
